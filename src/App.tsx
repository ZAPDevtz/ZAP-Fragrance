...
import { Menu, X, Sun, Moon, Heart, Sparkles as SparklesIcon, MessageCircle, Star } from 'lucide-react';
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[70] pointer-events-none">
            <motion.div
              ...
              className="w-full max-w-md pointer-events-auto px-4"
            >
              <div 
                className="bg-[var(--bg-primary)] p-8 md:p-12 shadow-2xl border text-center relative"
                style={{ borderColor: `${accentColor}40` }}
              >
                <button 
                  onClick={onClose} 
                  className="absolute top-4 right-4 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors duration-300 z-10"
                >
                  <X size={20} />
                </button>
                ...
                <div className="flex justify-center mb-6">
                  <div 
                    className="w-16 h-16 rounded-full flex items-center justify-center ring-1"
                    style={{ 
  ...
  return (
    <div className="relative h-screen flex flex-col justify-center items-center text-center px-6 overflow-hidden">
      ...
      >
        ...
        <h1 className="font-serif text-5xl md:text-7xl lg:text-8xl text-[var(--text-primary)] leading-tight mb-8">
          Sensory Memories, <br />
          <span className="italic relative inline-block" style={{ color: accentColor }}>
            Curated to Linger.
            <motion.span 
              initial={{ width: 0 }}
              animate={{ width: '100%' }}
              transition={{ delay: 1, duration: 1.5 }}
              className="absolute bottom-2 left-0 h-[1px] bg-current opacity-60"
            />
          </span>
        </h1>
        <p className="font-sans font-light text-[var(--text-secondary)] text-lg md:text-xl mb-12 max-w-xl mx-auto leading-relaxed">
          Guests may forget the menu, but they will always remember how your wedding smelled.
        </p>
        
        <div onClick={onOpenContact}>
          <Button className="shadow-lg">Request a Curated Experience</Button>
        </div>
      </motion.div>
    ...
    <Section id="intro" className="text-center">
      <div className="max-w-2xl mx-auto">
        <div className="inline-block mb-6">
           <div className="p-4 rounded-full border border-[var(--text-secondary)]/10">
              <SparklesIcon className="w-8 h-8 opacity-70" style={{ color: accentColor }} />
           </div>
        </div>
...
const ProcessStep = ({ number, title, desc }: { number: string; title: string; desc: string }) => {
  ...
  return (
    <div className="h-full group">
      <div className="flex flex-col items-center text-center space-y-3 p-8 bg-[var(--card-bg)] backdrop-blur-sm border border-[var(--card-border)] rounded-sm h-full transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-lg">
        <span className="font-serif text-5xl opacity-30 italic font-bold transition-opacity duration-300 group-hover:opacity-60" style={{ color: accentColor }}>{number}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-8 z-10">
          ...
          <div className="space-y-4">
            ...
            {/* Night Option */}
            <div onClick={() => setActiveMode('night')}>
              <div 
        ...
        <div className="relative h-[500px] md:h-[600px] w-full z-0">
          <div className="absolute inset-0 bg-stone-900/5 rounded-[2rem] transform translate-x-4 translate-y-4 -z-10" />
          
          <div className="relative h-full w-full rounded-[2rem] overflow-hidden shadow-xl border border-[var(--card-border)]">
  ...
  return (
    <Section className="">
      <div className="w-full">
        <div className="bg-[var(--card-bg)] backdrop-blur-sm rounded-lg py-16 px-8 border border-[var(--card-border)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              <ul className="space-y-4">
                <li className="flex items-center space-x-3 text-[var(--text-secondary)]">
                  <span className="w-2 h-2 rounded-full shadow-sm" style={{ backgroundColor: accentColor }}></span>
...
const TierCard = ({ title, sub, features, recommended = false }: { title: string, sub: string, features: string[], recommended?: boolean }) => {
  ...
  return (
    <div className="h-full group">
      <div className={`relative p-8 border ${recommended ? 'bg-[var(--card-bg)] shadow-lg' : 'border-[var(--card-border)] bg-[var(--card-bg)]'} flex flex-col h-full rounded-sm transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-xl`}
          style={recommended ? { borderColor: accentColor } : {}}
      >
        {recommended && (
          <div 
            className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-[10px] uppercase tracking-widest py-1 px-3 shadow-md z-20"
            style={{ backgroundColor: accentColor }}
          >
            Most Requested
          </div>
...
const WhyUs = () => {
  ...
  return (
    <Section className="bg-stone-900 text-ivory py-24 -mx-6 w-[calc(100%+3rem)] rounded-none z-20 relative">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
        <div>
          <Heart className="w-12 h-12 mx-auto mb-6" style={{ color: accentColor }} />
        ...
        <div>
          <Star className="w-12 h-12 mx-auto mb-6" style={{ color: accentColor }} />
        ...
        <div>
          <SparklesIcon className="w-12 h-12 mx-auto mb-6" style={{ color: accentColor }} />
...
const Content = () => {
  ...
  return (
    <div className="relative min-h-screen text-[var(--text-primary)] bg-[var(--bg-primary)] transition-colors duration-1000 overflow-x-hidden">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
